...
1. **Copy the env template**
   ...
   cp .env.example .env
   ```
2. **Create a Supabase (or Lovable Cloud) project** and paste its URL, publishable/anon key and project ID into `.env`. These `VITE_*` values are publishable — they are safe to ship in the browser bundle because Row Level Security protects the database.
