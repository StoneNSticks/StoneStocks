...
export function NewsList({ news }: { news: any[] }) {
  ...
  if (!news || news.length === 0) {
    return (
      <div className="rounded-xl border border-border/60 bg-card p-5">
        <h3 className="font-display font-semibold text-sm text-muted-foreground mb-3">{t("newsList.title")}</h3>
        <p className="text-sm text-muted-foreground">{t("newsList.none")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card p-5">
      <h3 className="font-display font-semibold text-sm text-muted-foreground mb-4">{t("newsList.latest")}</h3>
          ...
          <a key={i} href={item.url} target="_blank" rel="noopener noreferrer" className="flex items-start gap-3 rounded-lg p-2.5 -mx-2.5 transition-colors hover:bg-muted group">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-[11px] text-muted-foreground">{formatDate(new Date(item.datetime * 1000).toISOString())}</span>
