...
interface MarketInfo { name: string; timezone: string; openHour: number; openMin: number; closeHour: number; closeMin: number; flag: string; }
...
function getMarketTime(tz: string): Date { const str = new Date().toLocaleString("en-US", { timeZone: tz }); return new Date(str); }
...
function isMarketOpen(m: MarketInfo): boolean {
  ...
  return mins >= m.openHour * 60 + m.openMin && mins < m.closeHour * 60 + m.closeMin;
}
...
function timeUntil(m: MarketInfo): string {
  ...
  let diffMins: number;
  if (marketOpen) { diffMins = close - mins; }
  else if (isWeekend(now)) { const day = now.getDay(); const daysUntilMon = day === 0 ? 1 : 2; diffMins = daysUntilMon * 24 * 60 + (open - mins); if (diffMins < 0) diffMins += 24 * 60; }
  else if (mins < open) { diffMins = open - mins; }
  else { const nextDay = new Date(now); nextDay.setDate(nextDay.getDate() + 1); const daysToAdd = nextDay.getDay() === 6 ? 2 : nextDay.getDay() === 0 ? 1 : 0; diffMins = (1 + daysToAdd) * 24 * 60 - mins + open; }
...
export function MarketClock() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
        <Clock className="h-3.5 w-3.5" />
        <span className="tabular-nums hidden sm:inline">{localTime}</span>
        <Circle className={`h-2 w-2 fill-current ${anyOpen ? "text-gain" : "text-loss"}`} />
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1.5 w-72 max-w-[calc(100vw-2rem)] rounded-xl border border-border/60 bg-card shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="px-4 py-2.5 border-b border-border/40">
            <p className="text-xs font-semibold text-foreground">{t("clock.title")}</p>
            <p className="text-[10px] text-muted-foreground">{t("clock.yourTime")}: {localTime}</p>
          </div>
          <div className="py-1">
            {MARKETS.map((m) => {
              ...
              return (
                <div key={m.name} className="flex items-center gap-2.5 px-4 py-2 hover:bg-muted/50 transition-colors">
                  <span className="text-sm">{m.flag}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium truncate">{m.name}</p>
                    <p className="text-[10px] text-muted-foreground tabular-nums">{formatLocalTime(m.timezone)} {t("clock.local")}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="flex items-center gap-1">
                      <Circle className={`h-1.5 w-1.5 fill-current ${mOpen ? "text-gain" : "text-loss"}`} />
                      <span className={`text-[11px] font-semibold ${mOpen ? "text-gain" : "text-loss"}`}>{mOpen ? t("clock.open") : t("clock.closed")}</span>
                    </div>
                    <p className="text-[10px] text-muted-foreground tabular-nums">{mOpen ? `${t("clock.closesIn")} ${remaining}` : `${t("clock.opensIn")} ${remaining}`}</p>
                  </div>
                </div>
              );
            })}
