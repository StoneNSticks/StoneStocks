    ...
    <div className="rounded-xl border border-border/60 bg-card p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display text-lg font-semibold">{t("top.title")}</h2>
        </div>
        ...
        {companies.slice(0, 10).map((c: any, i: number) => (
          <Link key={c.symbol} to={`/stock/${c.symbol}`} className="flex items-center gap-3 rounded-lg p-2.5 -mx-1 transition-colors hover:bg-muted group">
            ...
            <div className="text-right flex-shrink-0">
              ...
              <div className={`text-[11px] font-medium ${priceChangeColor(c.changePercent)}`}>{formatPercent(c.changePercent)}</div>
            </div>
            <div className="text-right flex-shrink-0 hidden sm:block w-20">
              <div className="text-xs text-muted-foreground">{cs}{formatNumber(convert(c.marketCap))}</div>
            </div>
          </Link>
        ))}
      </div>
