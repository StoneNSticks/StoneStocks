  ...
  return (
    <div ref={ref} className={`relative ${compact ? "w-full" : "w-full max-w-xl mx-auto"}`}>
      ...
      {showRecent && (
        <div className="absolute top-full left-0 right-0 mt-1.5 rounded-xl border border-border bg-card shadow-xl z-50 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2 border-b border-border">
            <span className="text-xs font-medium text-muted-foreground">{t("search.recent")}</span>
            <button onClick={clearRecent} className="text-xs text-muted-foreground hover:text-foreground transition-colors">{t("search.clear")}</button>
          </div>
          <ul>
            {recentSearches.map((r) => (
              <li key={r.symbol}>
                <button onClick={() => handleSelect(r.symbol, r.name)} className="flex w-full items-center gap-3 px-4 py-2.5 text-left hover:bg-muted transition-colors">
      ...
      {showResults && (
        <div className="absolute top-full left-0 right-0 mt-1.5 rounded-xl border border-border bg-card shadow-xl z-50 overflow-hidden">
          {isLoading ? (
            <div className="p-3 text-sm text-muted-foreground text-center">{t("search.searching")}</div>
          ) : results && results.length > 0 ? (
            <ul className="max-h-72 overflow-y-auto">
              {results.map((r: any) => (
                <li key={r.symbol}>
                  <button onClick={() => handleSelect(r.symbol, r.description)} className="flex w-full items-center justify-between px-4 py-2.5 text-left hover:bg-muted transition-colors">
                    <div>
                      <span className="font-display font-semibold text-sm">{r.symbol}</span>
            ...
            <div className="p-3 text-sm text-muted-foreground text-center">{t("search.noResults")}</div>
          )}
        </div>
      )}
    </div>
