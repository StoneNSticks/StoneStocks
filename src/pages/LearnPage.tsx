          ...
          <div className="space-y-4">
            {tocGroups.map((group) => (
              <div key={group.title}>
                ...
                <nav className="grid sm:grid-cols-2 lg:grid-cols-3 gap-1 pl-2">
                  {group.items.map((item) => {
                    const sectionId = item.href.replace("#", "");
                    const isRead = read.includes(sectionId);
                    return (
                      <a key={item.href} href={item.href} className="flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-primary/5 transition-colors">
                        {isRead ? <CheckCircle className="h-3 w-3 text-chart-2 shrink-0" /> : <ArrowRight className="h-3 w-3 shrink-0" />}
                        {item.label}
                      </a>
                    );
                  })}
